...
export const SongInfoLink = (props: LinkProps) => {
  ...
  console.log("Link theme", theme.palette.text.primary, theme)
  return (
...
export const SongInfo = (props: SongInfoProps) => {
  ...
  const theme = useTheme();
  console.log("Songinfo theme", theme.palette.text.primary, theme)
        ...
        <StyledBox sx={{ display: "flex" }}>
          {artists.map(({ name, url }, i) => (
            <React.Fragment key={url}>
              ...
              <SongInfoLink key={url} href={url} color="text" variant="caption">{name}</SongInfoLink>
            </React.Fragment>
          ))}
        </StyledBox>
      
        {!hideAlbum && (
          <SongInfoLink href={album.url} color="text" variant="caption">{album.name}</SongInfoLink>
        )}
      </StyledBox>
    </StyledBox>
  );
